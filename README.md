# 🎬 유튜브 영상 자동 제작·배포 시스템

주제·형식·구성 방식·참고 URL을 입력하면 **대본 → 나레이션(TTS) → 비주얼 → 영상 조립 → 썸네일**까지 자동 생성하고,
사용자 확인 후 YouTube에 업로드하는 시스템입니다.

## 실행 방법

`run.bat` 더블클릭 → 브라우저에서 http://localhost:8600 자동 열림

## 웹(외부)에서 접속하기 — Cloudflare Tunnel

`tunnel.bat` 더블클릭 → 콘솔에 표시되는 `https://???.trycloudflare.com` 주소로 어디서든 접속.

- 접속 시 브라우저가 아이디/비밀번호를 물음 → **아이디는 아무거나, 비밀번호는 `.env`의 `APP_PASSWORD`**
- 임시 터널이라 **주소는 터널을 켤 때마다 바뀜** (PC와 터널이 켜져 있는 동안만 접속 가능)
- 파일 업로드는 요청당 최대 100MB (Cloudflare 무료 플랜 제한)
- 고정 주소가 필요하면: Cloudflare 계정 + 보유 도메인 등록 후 명명된 터널(named tunnel)로 전환
  (`cloudflared tunnel login` → `cloudflared tunnel create ...`), 이때 Cloudflare Access로 이메일 인증도 무료로 추가 가능

## 파이프라인

| 단계 | 사용 기술 | 비고 |
|---|---|---|
| 참고 영상 분석 | yt-dlp | 제목/설명/태그 추출 → 대본 스타일 참고 |
| 소스 자료 | 텍스트 입력 + 파일 업로드 | 텍스트는 대본의 최우선 근거, 이미지/영상은 장면 순서대로 사용, 오디오는 BGM(자동 볼륨 12%) |
| 대본 생성 | Claude API | 키 없으면 데모 대본으로 동작 |
| 나레이션 | edge-tts (무료) | 한국어 남/여 음성 선택 |
| 비주얼 | Pexels API(스톡영상 기반) / Pillow(그래픽 기반) | Pexels 키 없으면 슬라이드로 대체 |
| 자막 스타일 | 폰트 8종+커스텀 / 크기 3단계 / 색상 / 박스·외곽선·그림자 | 폰트는 슬라이드·썸네일에도 적용 |
| 영상 조립 | FFmpeg | 롱폼 1920×1080 / 숏폼 1080×1920, 자막 번인, SRT 별도 생성 |
| 썸네일 | Pillow | 영상 프레임 + 제목 문구 |
| 업로드 | YouTube Data API v3 | **버튼 클릭 + 확인 후에만 실행**, 기본 비공개 |

## 간편 영상 편집기

완성된 작업 카드의 **✂️ 편집하기** 버튼 → 타임라인 편집기 (또는 http://localhost:8600/edit 에서 빈 편집기로 시작).

- **자르기**: 클립마다 시작/끝 슬라이더로 사용할 구간만 남김
- **분할**: 미리보기를 원하는 위치에 멈추고 "현재 위치에서 분할" → 두 조각으로 나뉨
- **순서 변경**: ⠿ 드래그 앤 드롭 또는 ▲▼ 버튼
- **이어붙이기**: "클립 추가"로 외부 영상 업로드 → 타임라인 끝에 추가 (해상도 자동 맞춤, 무음 영상도 처리)
- **복제/삭제**, 총 길이 실시간 표시
- **전체 이어보기**: 렌더링 없이 타임라인 순서대로 미리 재생
- **자동 저장·이어서 편집**: 편집 상태가 자동 저장되고, /edit 에서 이전 세션 목록을 열어 이어서 작업
- 렌더링 후: 원본 작업에 적용(기존 업로드 화면 사용) 또는 편집기에서 바로 YouTube 업로드
- 원본 작업의 배경음악(BGM)은 재렌더 시 자동으로 다시 적용됨

## 자막 폰트·스타일

- **폰트**: 시스템에 설치된 한글 폰트 자동 탐색 (맑은 고딕, 한컴 고딕, 본고딕/본명조, 함초롬바탕, KoPub 바탕, 굴림, 바탕)
- **직접 추가**: 프로젝트의 `fonts/` 폴더에 .ttf/.otf 파일을 넣으면 목록에 나타남
- **크기**: 작게/보통/크게 · **색상**: 색상 선택기 · **스타일**: 반투명 박스 / 외곽선(예능 자막) / 그림자
- 선택한 폰트는 자막뿐 아니라 인포그래픽·카드뉴스 슬라이드와 썸네일 문구에도 적용
- **재렌더**: 완성된 작업 카드의 "🔁 재렌더" 패널에서 자막·BGM·나레이션 음성을 바꿔 다시 생성
  (대본은 유지, 음성을 바꾸면 TTS만 다시 실행되어 새 음성 길이에 맞춰 재조립)
- **썸네일 편집**: "🖼 썸네일 편집" 패널에서 문구·문구 색·프레임 위치(영상 0~95% 지점)·배경 이미지 업로드를
  바꿔 **영상 재렌더 없이 즉시** 썸네일만 재생성 (미리보기 포함, 설정은 저장되어 이후 재렌더에도 반영)

## 인트로 / 아웃트로

- **인트로 템플릿**: 타이틀 카드(채널명+영상 제목) / 미니멀(채널명) — 2.5초 자동 생성
- **아웃트로 템플릿**: 구독 유도(구독·좋아요 버튼 그래픽) / 감사 인사 — 3.5초 자동 생성
- **직접 업로드**: 자기 인트로/아웃트로 영상을 올리면 템플릿 대신 사용 (영상의 소리 유지, 해상도 자동 맞춤)
- 채널명 입력 시 템플릿에 표시, 자막 폰트 설정이 템플릿에도 적용, 화면 전환·효과음도 경계에 적용
- 생성 폼과 재렌더 패널 양쪽에서 변경 가능 (편집기에서는 인트로/아웃트로도 개별 클립으로 편집 가능)

## 워터마크 / 채널 로고

- **유형**: 로고 이미지 업로드(투명 PNG 권장) 또는 채널명 텍스트(흰 글씨+외곽선 자동 생성)
- **위치** 4곳(우상단 기본) / **크기** 3단계 / **투명도** 10~100% 슬라이더
- 인트로부터 아웃트로까지 영상 전체에 적용, 페이드 인/아웃과 함께 자연스럽게 처리
- 생성 폼·재렌더 패널에서 설정, 편집기 렌더 결과에도 유지

## 화면 전환·효과음

- **화면 전환** (6종): 없음 / 디졸브 / 페이드 블랙 / 슬라이드 / 와이프 / 원형 확대
  - 각 장면 앞에 전환 길이(0.4초)만큼 무음 리드인을 넣어 **나레이션이 잘리지 않음**
- **전환 효과음** (자체 합성음, 저작권 무관): 휙(스우시) / 딩(벨) / 팝 — 장면 경계마다 자동 삽입
- 생성 폼과 재렌더 패널 양쪽에서 선택 가능

## 배경음악 (BGM)

- 생성 시: 오디오 파일 업로드(여러 개 가능) + **볼륨 슬라이더(0~50%, 기본 12%)**
- 재렌더 시: 업로드된 오디오 중 **다른 파일로 교체 / BGM 제거 / 새 파일 업로드 / 볼륨 변경**
- 여러 오디오를 올려두면 기본으로 첫 번째가 사용되고, 재렌더에서 파일별로 바꿔가며 비교 가능

## 구성 방식 (8종)

**스톡영상 기반** (Pexels 영상 + 오버레이):
| 방식 | 화면 구성 |
|---|---|
| 실사 | 스톡영상 + 하단 자막 |
| 다큐/스토리텔링 | 시네마틱 레터박스(상하 블랙바) + 중앙 자막, 서정적 나레이션 |
| 뉴스 클립 | 하단 헤드라인 자막바("속보" 태그), 보도체 나레이션 |
| 랭킹 Top N | 좌상단 "TOP 3" 뱃지, 카운트다운 구성(마지막이 1위) |
| 튜토리얼 | 좌상단 "STEP 1" 뱃지, 단계별 하우투 구성 |

**그래픽 기반** (Pillow 자동 생성 슬라이드, Pexels 불필요):
| 방식 | 화면 구성 |
|---|---|
| 인포그래픽 | 제목 + 불릿 요약 슬라이드 |
| 카드뉴스 | 밝은 배경 + 중앙 대형 문구 카드 |
| 명언/감성 | 어두운 배경 + 큰따옴표 + 중앙 문구 |

## API 키 설정 (.env)

`.env.example`을 복사해 `.env` 생성 후 키 입력:

```
copy .env.example .env
```

### 1. Anthropic (대본 생성 — 필수에 가까움)
- https://console.anthropic.com → API Keys 발급 → `ANTHROPIC_API_KEY=`
- 없으면 데모 대본으로 파이프라인 테스트만 가능

### 2. Pexels (실사/뉴스 스톡영상 — 무료)
- https://www.pexels.com/api/ 가입 → 키 발급 → `PEXELS_API_KEY=`
- 없으면 그라디언트 슬라이드 배경으로 대체됨

### 3. YouTube 업로드 (OAuth)
1. https://console.cloud.google.com → 프로젝트 생성
2. "API 및 서비스" → **YouTube Data API v3** 사용 설정
3. "OAuth 동의 화면" → 외부 → 본인 계정을 **테스트 사용자**로 추가
4. "사용자 인증 정보" → OAuth 클라이언트 ID → **데스크톱 앱** → JSON 다운로드
5. 다운로드한 파일을 프로젝트 폴더에 `client_secret.json`으로 저장
6. 첫 업로드 시 브라우저 인증 1회 → 이후 `token.json` 자동 재사용

> 참고: 신규 OAuth 앱(테스트 모드)으로 업로드한 영상은 심사 전까지 **비공개로 잠길 수 있습니다.**
> 썸네일 설정은 채널 전화번호 인증이 되어 있어야 합니다.
> API 기본 할당량으로 하루 약 6개까지 업로드 가능합니다(영상 1개 = 1600 유닛 / 10000 유닛).

## 결과물 위치

`output/<작업ID>/` — `final.mp4`, `thumbnail.jpg`, `script.json`, `subtitles.srt`

## 구조

```
app.py               # FastAPI 서버 + API
static/index.html    # 웹 UI
pipeline/
  runner.py          # 파이프라인 오케스트레이션
  reference.py       # 참고 영상 분석 (yt-dlp)
  script_gen.py      # 대본 생성 (Claude)
  tts.py             # 나레이션 (edge-tts)
  visuals.py         # 스톡영상/슬라이드/자막 오버레이
  assemble.py        # FFmpeg 조립
  thumbnail.py       # 썸네일
  upload.py          # YouTube 업로드
```
